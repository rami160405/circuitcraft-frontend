...
// API base URL - automatically detect if using Live Server and route to Express server
const getBackendUrl = (path) => {
  const currentPort = window.location.port;
  // If running on Live Server (port 5500) or any port other than 5000, use full URL to port 5000
  if (currentPort && currentPort !== "5000" && currentPort !== "") {
    return `http://localhost:5000${path}`;
  }
  // Otherwise use relative path (when served by Express on port 5000)
  return path;
};
